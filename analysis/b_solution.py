...
def analyse_material(material: str, indices: tuple[int, int], band: tuple[float, float]) -> dict:
    ...
    for row, (r, fit1, fit3) in enumerate(zip(records, one["fits"], three["fits"])):
        ax = axes[row, 0]
        ax.plot(r["x_all"], r["y_all"], lw=0.7, color="#335c81")
        ...
        ax.plot(r["x"][::stride], r["y"][::stride], lw=0.8, color="#444444", label="实测")
        ax.plot(r["x"][::stride], fit1[::stride], lw=1.1, color="#2a9d8f", label="单谐波")
        ax.plot(r["x"][::stride], fit3[::stride], lw=1.0, color="#d1495b", label="三谐波")
        ax.set(title=f"拟合频段与模型对比（{r['angle']:.0f}°）", xlabel="波数 / cm$^{-1}$", ylabel="反射率 / %")
        ax.legend(frameon=False, ncol=3, fontsize=9)
    fig.savefig(OUT_DIR / f"{material}_spectral_fit.png", dpi=200)
    plt.close(fig)
    return result
