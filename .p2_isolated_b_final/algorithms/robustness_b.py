...
def fixed_design_error(record: dict, harmonics: int, initial: float, folds: int = 5) -> float:
    ...
    for valid in fold_ids:
        ...
        purge_lo = max(0, int(valid[0]) - 96)
        purge_hi = min(n, int(valid[-1]) + 97)
        train[purge_lo:purge_hi] = False
        bounds = (5.0, 10.0) if record["material"] == "SiC" else (2.0, 6.0)
        ...
        train_scale = max(float(np.std(record["y"][train])), 1e-12)
        squared += float(error @ error) / train_scale**2
        count += len(valid)
...
def make_figures(records: list[dict], boot: dict[int, np.ndarray], summary: pd.DataFrame, sensitivity_df: pd.DataFrame) -> None:
    ...
    fig, ax = plt.subplots()
    for r, color, line_style in zip(records[:2], COLORS, ["-", "--"]):
        ax.plot(r["x_all"], 100 * r["y_all"], color=color, ls=line_style, lw=0.8, label=f"{r['angle']:.0f}°")
    ...
    fig, ax = plt.subplots()
    line_styles = ["-", "--", "-.", ":"]
    for r, color, line_style in zip(records, COLORS, line_styles):
        z, _ = preprocess(r["x"], r["y"])
        ax.plot(r["x"], z, color=color, ls=line_style, lw=0.75, alpha=0.85, label=f"{r['material']} {r['angle']:.0f}°")
    ...
    # Process evidence.
    fig, ax = plt.subplots()
    for r, color, line_style in zip(records, COLORS, ["-", "--", "-.", ":"]):
        q = optical_coordinate(r["x"], r["angle"], r["material"])
        ax.plot(r["x"], q / 1e4, color=color, ls=line_style, lw=1.0, label=f"{r['material']} {r['angle']:.0f}°")
    ...
    ax.set(ylabel="相对基准厚度变化 (%)", title=f"相邻阶厚度序列区块 Bootstrap（每谱 {BOOTSTRAP_REPEATS} 次）")
    ...
    ax.set(ylabel=r"厚度 ($\mu$m)", title="各附件厚度与相邻阶序列 Bootstrap 95% 区间")
    ...
    markers = ["o", "s", "^", "D"]
    for (record_name, group), color, marker in zip(sensitivity_df.groupby("record"), COLORS, markers):
        delta = 100 * (group["thickness_um"] / group["base_um"] - 1)
        ax.scatter(group["factor_zh"], delta, label=record_name, color=color, marker=marker, alpha=0.8)
...
def main() -> None:
    ...
    bootstrap_rows = []
    for r in records:
        bootstrap_rows.extend({"record": f"附件{r['index']}({r['angle']:.0f}°)", "repeat": repeat + 1, "thickness_um": value} for repeat, value in enumerate(boot[r["index"]]))
    final_rows = []
    joint_rows = []
    for material, group in summary.groupby("material", sort=False):
        ...
        joint_rows.extend({"material": material, "repeat": repeat + 1, "joint_thickness_um": value} for repeat, value in enumerate(joint))
    ...
    pd.DataFrame(bootstrap_rows).to_csv(RESULT_ROOT / "bootstrap_repeats.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(joint_rows).to_csv(RESULT_ROOT / "bootstrap_joint_repeats.csv", index=False, encoding="utf-8-sig")
    make_figures(records, boot, summary, sensitivity_df)
    manifest = {
        "seed": SEED,
        "bootstrap_repeats": BOOTSTRAP_REPEATS,
        "bootstrap_method": "moving-block bootstrap of adjacent-order local thickness sequence",
        "block_length_local_orders": BLOCK_LENGTH,
        "blocked_cv": {"folds": 5, "purge_gap_points_each_side": 96, "SiC_bounds_um": [5.0, 10.0], "Si_bounds_um": [2.0, 6.0]},
        "bands_cm-1": {"SiC": [1500.0, 4000.0], "Si": [1000.0, 4000.0]},
        ...
        "scripts": {str(path.relative_to(ROOT)): sha256(path) for path in [ROOT / "scripts" / "run_b.py", ROOT / "algorithms" / "solve_b.py", ROOT / "algorithms" / "robustness_b.py"]},
        "outputs": ["results/b_results.json", "results/b_summary.csv", "results/robustness_summary.csv", "results/sensitivity_details.csv", "results/final_recommendations.csv", "results/bootstrap_repeats.csv", "results/bootstrap_joint_repeats.csv"],
    }
    manifest["output_files"] = {name: {"bytes": (ROOT / name).stat().st_size, "sha256": sha256(ROOT / name)} for name in manifest["outputs"]}
